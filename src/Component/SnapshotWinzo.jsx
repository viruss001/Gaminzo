...
const SnapshotWinzo = ({ theme = "dark" }) => {
  ...
  return (
    <section
      ref={sectionRef}
      className={`relative w-full min-h-screen px-6 py-16 flex flex-col justify-center items-center overflow-hidden transition-colors duration-500 ${
        isDark ? "bg-gray-900 text-gray-100" : "bg-gray-50 text-gray-900"
      }`}
    >
      {/* Background Blurs (Theme Aware) */}
        ...
        <motion.div
          ...
          className={`absolute top-1/4 left-1/3 w-72 h-72 rounded-full blur-3xl ${
            isDark
              ...
              : "bg-gradient-to-r from-emerald-300/20 to-blue-400/20"
          }`}
        />
        <motion.div
          ref={blurRef2}
          className={`absolute bottom-1/3 right-1/3 w-96 h-96 rounded-full blur-3xl ${
            isDark
              ? "bg-gradient-to-r from-blue-500/20 to-emerald-400/20"
              : "bg-gradient-to-r from-blue-400/20 to-emerald-300/20"
          }`}
        />
      </div>
      ...
      <motion.p
        ref={textRef}
        className={`text-center max-w-4xl text-lg leading-relaxed transition-colors duration-500 ${
